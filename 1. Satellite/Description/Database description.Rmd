---
title:  "The Aerial Parking Lot dataset: A description"
subtitle:
author: "[Nisim Hurst](mailto:langheran@gmail.com)"
institution: "Instituto Tecnológico y de Estudios Superiores de Monterrey _ITESM_"
date: Thursday 26 July 2018
abstract: "The  Aerial Parking Space dataset (_APKLOT_) is intended for two audiences: algorithm designers and researchers who want to validate the state of the art measured by performance on the Parking Space dataset."
urlcolor: blue
...
Even though there are many datasets like PKLot [@almeida2015] or CNRPark-EXT [@amato2017] for occupancy detection in parking lots, none of them were designed to tackle the parking block segmentation problem directly. Given the lack of a suitable dataset in the deep learning realm, we propose APKLOT, a dataset of roughly 7000 polygons for segmenting parking blocks from the satellite perspective and from the camera perspective.
...
```{r fig1, out.width="45%", fig.align="center", fig.cap="Example segmentation images on the Pascal VOC: (A) \\textbf{JPEGImages} folder, (B) \\textbf{SegmentationClass} folder, (C) \\textbf{SegmentationObject} folder. ", echo=FALSE}
knitr::include_graphics("images/apklot_examples/seg.png")
```
...
```{r fig2, out.width="90%", fig.align="center", fig.cap="Stats features from the APKLOT dataset: (A) size in KB, (B) width and height of the image, (C) total area vs annotated area, (D) area count per image. ", echo=FALSE}
...
```{r fig3, out.width="65%", fig.align="center", fig.cap="Example segmentation images on our dataset \\emph{APKLOT}. ", echo=FALSE}
knitr::include_graphics("images/segmentation_examples/seg.png")
```
