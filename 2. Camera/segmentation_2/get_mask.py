...
def GenerateFile(file, image_root_path):
    ...
    _shapes = [shape["points"]
               for shape in _data["shapes"] if shape["label"] == "1"]
    ...
    for _shape in _shapes:
        ...
        _shape = [[int(round(pt[0], 0)), int(round(pt[1], 0))]
                  for pt in _shape]
...
ap.add_argument("-i", "--input", required=False, help="Path to the image")
args = vars(ap.parse_args())
input = args["input"]
if (input is None):
    filelist = [f for f in os.listdir(os.getcwd()) if f.find('.png') != -1]
